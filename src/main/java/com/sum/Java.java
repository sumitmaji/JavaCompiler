...
import com.sum.backend.Backend;
import com.sum.backend.BackendFactory;
...
//			backend.addMessageListener(new BackendMessageListener());
		...
		if(dir.isDirectory()){
			...
			for (File file : files) {
				if(file.isDirectory())
					...
					new Java("Compile", file.getAbsolutePath(), "x");
	...
	public static void listFiles(File dir, String fileName){
		if(dir.isDirectory()){
			File[] files = dir.listFiles();
			for (File file : files) {
				if(file.isDirectory()) {
				...
				else if(file.getAbsolutePath().toLowerCase().contains("/" + fileName + ".java")){
					System.out.println("parsing file" + file.getName());
					new Java("Compile", file.getAbsolutePath(), "x");
				}
